...
import { LogIn, LogOut, ShieldCheck, LayoutDashboard, Home } from 'lucide-react'
        ...
        <Auth0Provider>
          <div className="flex min-h-full flex-col">
            ...
            <footer className="border-t border-gray-200 py-6 text-center text-xs text-gray-400">
              Built with <span className="font-medium">@auth0/auth0-tanstack-start-react</span>
            </footer>
          </div>
        </Auth0Provider>
        <TanStackDevtools
...
function NavLink({
  ...
  return (
    <Link
      to={to}
      className="inline-flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium text-gray-600 transition-colors hover:bg-gray-100 hover:text-gray-900 [&.active]:bg-indigo-50 [&.active]:text-indigo-700"
    ...
    <header className="sticky top-0 z-10 border-b border-gray-200 bg-white/80 backdrop-blur">
      <div className="mx-auto flex h-16 w-full max-w-5xl items-center gap-2 px-6">
        <Link to="/" className="mr-2 flex items-center gap-2 font-semibold">
          ...
          <span className="hidden sm:inline">Auth0 · TanStack Start</span>
        </Link>
